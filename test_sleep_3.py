...
def face_detection_run1():
    global under_score_count,running2,face_detection_switch,paizhao_voice_command,photo_base64,generate_photo_base64,last_x,last_y,continue_shot_and_record,photo_base64_list
    ...
    while running2 and face_detection_switch:
        ...
        if paizhao_voice_command:
            ...
            if retval:
                ...
                with threading.Lock():
                    subprocess.run(
                        ["aplay", "-D", "plughw:2,0", "-f", "S16_LE", "-r", "16000", "-c", "1", take_photo_file])
...
# def record_2s():
#     global continue_shot_and_record, start_record_time2, audio_memory2, message_id2, pre_buffer2, audio_data_queue2
#
#     # 录音开始前的一些设置
#     def start_new_recording():
#         global audio_memory2, start_record_time2
#         print("@@@@@@@@@@@@开始录音2......")
#         recording2 = True
#         start_record_time2 = time.time()
#
#         # 创建一个 BytesIO 对象来存储音频数据
#         # audio_memory2 = io.BytesIO()
#
#         # 将预缓冲中的数据写入
#         for frame in pre_buffer2:
#             audio_memory2.write(frame.tobytes())
#         pre_buffer2.clear()
#
#     def save_audio():
#         """保存录制的音频到文件"""
#         global audio_memory2, message_id2
#         denoise_output_file_name = f"/home/li/vosk-api/python/example/shot_and_recording/recording{message_id2}.wav"
#         audio_data = audio_memory2.getvalue()
#         audio_memory2.close()  # 关闭内存流
#         audio_memory2 = None
#
#         # 写入 .wav 文件
#         with wave.open(denoise_output_file_name, 'wb') as wav_file:
#             wav_file.setnchannels(1)  # 单声道
#             wav_file.setsampwidth(2)  # 16 位深度 (2 字节)
#             wav_file.setframerate(16000)  # 设置采样率（根据需要修改）
#
#             # 写入音频数据
#             wav_file.writeframes(audio_data)
#
#         print(f"保存音频2: {denoise_output_file_name}")
#
#     # 监听录音指令
#     while True:
#         if continue_shot_and_record:
#             print("Enter continue_shot_and_record!")
#             current_time = time.time()
#             print("current_time:",current_time)
#             # 每2秒保存一段音频
#             if current_time - start_record_time2 >= 2:
#                 print("save audio every 2s!")
#                 save_audio()
#
#                 # 开始新的录音
#                 message_id2 += 1
...
def create_connection_with_retries(url, max_retries=100, retry_interval=0.3):
    global ws
    ...
    while retries < max_retries:
        try:
            ...
            if retries < max_retries:
                ...
                raise e
    return None
def create_connection_with_retries2(url, max_retries=100, retry_interval=0.1):
    global ws2
    """尝试建立 WebSocket 连接，如果断开则重试"""
    retries = 0
    while retries < max_retries:
...
def continue_send_photo():
    global flag_error, audio_file_stack_length, paizhao_voice_command, \
        ...
        shield_after_send, flag_think2, old_message_id, photo_base64_list
    # WebSocket 地址
    retries = 0
    max_retries = 100
    retry_interval = 0.3
    ...
    while True:
        ...
        if c_count>=2 and len(photo_base64_list) >= 3:
...
def play_haode_function():
    ...
    while 1:
        ...
        if play_haode == True and not nihao_detected and not xiaoqi_detected:
            ...
            with threading.Lock():
                subprocess.run(
                ["aplay", "-D", "plughw:2,0", "-f", "S16_LE", "-r", "16000", "-c", "1", pcm_files[index]])
...
def play_qibaozaine_function():
    ...
    while 1:
        ...
        if play_qibaozaina == True and not nihao_detected and not xiaoqi_detected:
            ...
            with threading.Lock():
                subprocess.run(
                    ["aplay", "-D", "plughw:2,0", "-f", "S16_LE", "-r", "16000", "-c", "1", qibaozaine_file])
...
def mix_play():
    ...
    while True:
        try:
            local_output_file_name = play_queue.get(timeout=5)
...
#             pygame.mixer.music.load(bmg_file_name)
#             pygame.mixer.music.play(-1)
#             print(f"背景音乐 {bmg_file_name} 正在播放")
#         else:
#             print(f"背景音乐文件未找到: {bmg_file_name}")
#             exit()
#
#         send_mp3_thread = threading.Thread(target=send_mp3, daemon=True)
#         send_mp3_thread.start()
#
#         mix_play_thread = threading.Thread(target=mix_play, daemon=True)
#         mix_play_thread.start()
#
#         while True:
#             time.sleep(1)
  ...
  # 清除事件信号
    while running2:
        ...
        if len(output_file_stack) > 0 :
            # 从栈中取出文件
            local_output_file_name = output_file_stack.pop(0)
            ...
            if li_wait_time>0:
                print("li_wait_time1:",li_wait_time)
                # time.sleep(li_wait_time)
                time.sleep(0.5)
            play_queue.put(local_output_file_name)
        else:
...
def send_audio_request2(audio_data):
    global last_t1,t1,last_t2,t2,flag_error,audio_file_stack_length,paizhao_voice_command,\
        photo_base64,generate_photo_base64,nihao_detected,xiaoqi_detected,result_detected,\
        hello_detected,file_counter,message_id,resp_code_id,\
        conversation_id,output_audio_stack,recording,ws2,shield_after_send,flag_think,flag_think2,old_message_id,retries ,max_retries
    # WebSocket 地址
    ...
    while True:  # 无限重连直到成功
        try:
            if ws2 is None or not ws2.connected:
                print("WebSocket not connected. Attempting to reconnect...")
                ws = websocket.create_connection(websocket_url)
                print("WebSocket reconnected successfully!")
...
# def receive_execute_command():
#     global last_t1, t1, last_t2, t2, flag_error, audio_file_stack_length, paizhao_voice_command, \
#         photo_base64, generate_photo_base64, nihao_detected, xiaoqi_detected, result_detected, \
#         hello_detected, file_counter, message_id, resp_code_id, \
#         conversation_id, output_audio_stack, recording, ws, shield_after_send, flag_think, flag_think2, old_message_id
#
#     while True:
#         print("print receive_execute_command")
#         time.sleep(0.005)
#         try:
#             response = ws.recv()
#             if response is not None:
#                 print("receive_audio_request.....")
#                 print("response_audio_request time:", time.time())
#             response_data = json.loads(response)
...
def deal_received_audio_request():
    global last_t1, t1, last_t2, t2, flag_error, audio_file_stack_length, paizhao_voice_command, \
        ...
        last_li_scene_seq,li_wait_time,scent_spray_flag,bgm_num,standup_flag
    li_voices_list=[]
    ...
    while True:
        ...
        if response_data['method'] == 'execute-command':
            ...
            if response_data.get("code") == 0:
                ...
                print("last_li_scene_seq:",last_li_scene_seq)
                # if li_scene_seq==last_li_scene_seq+1:
                if li_scene_seq is not None and li_scene_seq > last_li_scene_seq :
                # if 1 >= 0:
                    print("yes")
                    ...
                    if li_voice is not None:
                        ...
                        for i in range (len(li_voices_list)):
                            print("i:", i)
                            ...
                            # print("li_audio_base64:", li_audio_base64)
                            if li_audio_base64 != None:
                                ...
                                if str(li_audio_fomat) == 'mp3':
                                    ...
                                    # time.sleep(5)
                                    output_file_stack.append(fixed_audio_file_name)
                                    print("fixed_audio_file_name appended!")
                ...
                last_li_scene_seq = li_scene_seq
                # last_li_scene_seq+=1
                ...
                if audio_base64 != None:
                    ...
                    if str(resp_fomat) == 'mp3':
                        ...
                        with open(output_file_name, "wb") as pcm_file:
                            pcm_file.write(audio_bytes)
                        print("Saved:", output_file_name)
                        output_file_stack.append(output_file_name)
                        output_file_stack_length = len(output_file_stack)
                        print("播放栈len:", output_file_stack_length)
                        ...
                        # player = subprocess.Popen(["aplay", "-D", "plughw:5,0", output_wav])
                        # player = subprocess.Popen(["mpg123", "-o", "alsa:plughw:3,0", output_file_name])
