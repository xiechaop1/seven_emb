        ...
        if c_count>=2 and len(photo_base64_list) >= 3:
            ...
            with open("play_status.txt", "r") as file_status:
                ...
                if content2 == 'None' or content2 == '':
                    ...
                    play_status2 = None  # 如果文件为空，可以设置默认值，或者根据需求处理
                else:
...
def mix_play():
    global recording_event,nihao_detected,xiaoqi_detected,local_output_file_name_list,standup_flag,\
        li_voices_list_len,play_status,interrupt_flag,li_wait_time,is_garbage_content
    ...
    while True:
        ...
        if os.path.exists(local_output_file_name):
            ...
            try:
                ...
                print("wait end!!!")
                while voice_channel.get_busy() or (pygame.time.get_ticks() - start_time < li_wait_time * 1000):
                    ...
                    if resp_message_id != message_id :
                        ...
                        if is_garbage_content==True:
                            ...
                            voice_channel.play(voice, loops=0, start=interrupted_position)
                            print("keep playing from current position")
...
def deal_received_audio_request():
    global last_t1, t1, last_t2, t2, flag_error, audio_file_stack_length, paizhao_voice_command, \
        ...
        qibao_sentence_complete,li_voice,interrupt_flag,cant_connect_flag,li_filename,sleep_mode
    li_voices_list=[]
                ...
                if cant_connect_flag == True:
                    ...
                    if response_data.get("code") == 0:
                        ...
                        if li_scene_seq is not None:
                            print("old_seq<100 and (li_scene_seq > old_seq or (interrupt_flag==True and li_scene_seq==old_seq)) and qibao_sentence_complete:",old_seq<100 and (li_scene_seq > old_seq or (interrupt_flag==True and li_scene_seq==old_seq and qibao_sentence_complete)))
                            print("(old_seq>100 and li_scene_seq!=old_seq) or (interrupt_flag==True and li_scene_seq==old_seq and qibao_sentence_complete):",(old_seq>100 and li_scene_seq!=old_seq and qibao_sentence_complete) or (interrupt_flag==True and li_scene_seq==old_seq and qibao_sentence_complete))
                            if (old_seq<100 and (li_scene_seq > old_seq or (interrupt_flag==True and li_scene_seq==old_seq and qibao_sentence_complete))) or ((old_seq>100 and li_scene_seq!=old_seq and qibao_sentence_complete) or (interrupt_flag==True and li_scene_seq==old_seq and qibao_sentence_complete)):
                                with open("li_scene_seq.txt", "w") as file_scene:
                                    file_scene.write(str(li_scene_seq))  # 将li_scene_seq写入文件，确保是数字序号的字符串格式
                            # if 1 >= 0:
                                print("yes")
