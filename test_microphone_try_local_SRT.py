...
def create_connection_with_retries(url, max_retries=100, retry_interval=0.1):
    global ws
...
#                '小觑', '小曲', '小心']
xiaoqi_word=['细胞','七宝','鸡毛','稀薄','七','旗袍','确保','祈祷']
stop_word = ['停', '停止']
paizhao_word = ['拍', '拍照']
...
def playback_thread():
    global nihao_detected, xiaoqi_detected, player2,message_id, output_audio_stack, nihao_event, running2,recording,\
        play_haode,audio_file_stack_length,xiaoqi_event,play_haode_event,recording_event,shield_after_send,flag_think,resp_message_id
    chunk_duration = 0.1  # 每次播放 0.1 秒的音频
    sample_rate = 16000  # 音频采样率
    channels = 1  # 单声道
    bytes_per_sample = 2  # 每个样本 2 字节（16 位 PCM）
    chunk_size = int(sample_rate * bytes_per_sample * channels * chunk_duration*4)  # 每块音频的大小
  ...
  # 清除事件信号
    while 1:
        ...
        print("enter playback_thread!")
        print("len(output_audio_stack):",len(output_audio_stack))
...
#         is_silent_flag,recording_event,flag_think,old_message_id,last_message_id
...
shm_size = 1024
shm_name_volume = "/shared_memory_volume"  # 音量共享内存名称
shm_size_volume = 4
cleaned_data=""
def receive_from_cpp():
    global cleaned_data,xiaoqi_word,xiaoqi_event,xiaoqi_detected,audio_file_stack,message_id,play_haode
    filter_words = ["嗯", "啊", "哦", "呃", "唔", "喔", "啊哈", "呐", ]
    print("Enter receive_from_cpp!!!")
    # 创建一个正则表达式，用于匹配无意义的词
    filter_pattern = re.compile(r'(' + '|'.join(map(re.escape, filter_words)) + r')+')

    # 打开共享内存（与C++程序中的 shm_open 相对应）
    with open("/dev/shm" + shm_name, "r+b") as f,open("/dev/shm" + shm_name_volume, "r+b") as f_volume:
        # 创建内存映射
        shared_memory = mmap.mmap(f.fileno(), shm_size)
        shared_memory_volume = mmap.mmap(f_volume.fileno(), shm_size_volume)
        # 清除共享内存中的内容
        # shared_memory[:] = b'\x00' * shm_size  # 重置共享内存内容为零
        # 循环接收数据
        while True:
            # 读取共享内存中的数据
            raw_data = shared_memory[:1024]
            # print("Raw data:", list(raw_data))
            data = raw_data.decode('utf-8', errors='ignore').strip('\x00')
            filtered_data = filter_pattern.sub('', data)
            filtered_data = re.sub(r'\s+', ' ', filtered_data).strip()
            # print("Received_filtered_data:", filtered_data)
            # 读取音量共享内存中的数据
            raw_volume_data = shared_memory_volume[:shm_size_volume]
            volume1 = 100*struct.unpack('f', raw_volume_data)[0]  # 将二进制数据转换为float
            print("volume1:",volume1)

            if data:
                # 过滤掉无意义的词
                cleaned_data = filter_pattern.sub('', data)

                if cleaned_data is not None and len(cleaned_data)>=2:
                    ...
                    audio_file_stack.append(cleaned_data)
                    # send_audio_request(cleaned_data)
                ...
                byte_data = raw_data.rstrip(b'\x00')  # 去除尾部空字节

                # 将处理后的字节数据解码
                cleaned_data = byte_data.decode('utf-8', errors='ignore')
                # print("Cleaned data:", cleaned_data)
...
if __name__ == "__main__":
    ...
    try:
        ...
        receive_from_cpp_thread = threading.Thread(target=receive_from_cpp)
        receive_from_cpp_thread.start()
