    ...
    while running2 and face_detection_switch:
        ...
        if paizhao_voice_command:
            ...
            if retval:
                ...
                with threading.Lock():
                    subprocess.run(
                        ["aplay", "-D", "plughw:5,0", "-f", "S16_LE", "-r", "16000", "-c", "1", take_photo_file])
...
def create_connection_with_retries(url, max_retries=100, retry_interval=0.5):
    global ws
...
import random
qibaozaine_file=f"/home/orangepi/vosk-api/python/example/qibaozaine.pcm"
...
def play_qibaozaine_function():
    ...
    while 1:
        ...
        if play_qibaozaina == True and not nihao_detected and not xiaoqi_detected:
            ...
            with threading.Lock():
                subprocess.run(
                    ["aplay", "-D", "plughw:5,0", "-f", "S16_LE", "-r", "16000", "-c", "1", qibaozaine_file])
...
def playback_thread():
    global nihao_detected, xiaoqi_detected, player2,message_id, output_audio_stack, nihao_event, running2,recording,\
        play_haode,audio_file_stack_length,xiaoqi_event,play_haode_event,play_qibaozaina_event,recording_event,shield_after_send,flag_think,resp_message_id
    ...
    # play_qibaozaina_event.wait()
    # play_qibaozaina_event.clear()  # ��除事件信号
  # 清除事件信号
    while running2:
        time.sleep(0.001)
...
def deal_received_audio_request():
    ...
    while True:
        ...
        print("deal_received_audio_request::response_data:",response_data)
        if response_data['method'] == 'voice-chat':
            if response_data.get("code") == 0:
                # 提取并解码音频数据
                print("response_data recieived:......")
