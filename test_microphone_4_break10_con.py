...
def speed_control(speed,speed2,step):
    ...
    for i in range(int(step*3)):
        ...
        if i % 2 == 0:
            ...
            # print("current_pulse_width2:",current_pulse_width2)
start_angel1=1300
start_angel0=1300
def init():
...
def face_detection_run1():
    global running2,face_detection_switch,paizhao_voice_command,photo_base64,generate_photo_base64,last_x,last_y
    ...
    while running2 and face_detection_switch:
        ...
        center_x_min = 234 - 2
        center_x_max = 254 + 2
        center_y_min = 112 - 2+60
        center_y_max = 132 + 2+60
        # center_x_min = 604
        # center_x_max = 644
        # center_y_min = 415
        # center_y_max = 455
...
video_path = '/home/orangepi/blink2.mp4'
video_path2 = '/home/orangepi/think.mp4'
video_path3 = '/home/orangepi/loading.mp4'
...
def send_photo_request(photo_base64):
    global flag_error, audio_file_stack_length, paizhao_voice_command,\
        ...
        conversation_id,ws2,flag_think2
    # WebSocket 地址
...
def play_haode_function():
    ...
    while 1:
        ...
        if play_haode == True and not nihao_detected and not xiaoqi_detected:
            ...
            with threading.Lock():
                subprocess.run(
                ["aplay", "-D", "plughw:5,0", "-f", "S16_LE", "-r", "16000", "-c", "1", haode_file])
...
def stop_player(player):
    ...
    if player:
        try:
            player.stdin.close()  # 关闭标准输入流
            player.wait()  # 等待播放器进程退出
        except Exception as e:
            print(f"Error during stopping player: {e}")
        finally:
            player.terminate()  # 确保终止播放器进程
def start_player():
...
def playback_thread():
    global nihao_detected, xiaoqi_detected, player2,message_id, \
        ...
        play_haode,audio_file_stack_length,xiaoqi_event,sleep_event,play_haode_event,\
        recording_event,shield_after_send,flag_think,sleep_detected,currently_playing
    chunk_duration = 0.1  # 每次播放 0.1 秒的音频
    sample_rate = 16000  # 音频采样率
    channels = 1  # 单声道
    bytes_per_sample = 2  # 每个样本 2 字节（16 位 PCM）
    chunk_size = int(sample_rate * bytes_per_sample * channels * chunk_duration*2)  # 每块音频的大小
  ...
  # 清除事件信号
    while running2:
        ...
        if len(output_audio_stack) > 0 :
            ...
            with player_lock:
                if player2 is None or player2.poll() is not None:
                    print("Player is closed or terminated, restarting...")
                    if player2 is not None:
                        try:
                            player2.stdin.close()  # 关闭标准输入流
                            player2.wait()  # 等待进程退出
                        except Exception as e:
                            print(f"Error closing previous player: {e}")
                    player2 = start_player()

            if shield_after_send==False and (no_match_but_valid or nihao_detected or xiaoqi_detected or sleep_detected or audio_file_stack_length > 1):
            ...
            try:
                while True:
                    if player2.stdin.closed:
                        with player_lock:
                            print("Player's stdin is closed, restarting player...")
                            if player2 is not None:
                                try:
                                    player2.stdin.close()  # 关闭标准输入流
                                    player2.wait()  # 等待进程退出
                    ...
                    try:
                        ...
                        with player_lock:
                            print("Broken pipe error occurred, restarting player...")
                            if player2 is not None:
                                try:
                                    player2.stdin.close()  # 关闭标准输入流
                                    player2.wait()  # 等待进程退出
                                    ...
                                    print(f"Error closing previous player: {e}")
                            player2 = start_player()  # 重新启动播放器
                            player2.stdin.write(chunk)  # 尝试再次写入数据

                    # 检查中断信号
                    if shield_after_send==False and (recording_event.is_set() or sleep_event.is_set() or xiaoqi_event.is_set() or nihao_detected or sleep_detected or xiaoqi_detected or no_match_but_valid or audio_file_stack_length > 1):
                        print("$$$$$$$$$$$$$$$$$")
                        print("sleep_event.is_set():", sleep_event.is_set())
...
def send_audio_request(audio_file_path):
    global last_t1,t1,last_t2,t2,flag_error,audio_file_stack_length,paizhao_voice_command,\
        ...
        hello_detected,file_counter,message_id,no_match_but_valid,resp_code_id,\
        conversation_id,output_audio_stack,recording,ws,shield_after_send,flag_think2,sleep_detected
    # WebSocket 地址
    ...
    try:
        ...
        while True:
            ...
            if no_match_but_valid == True or nihao_detected or xiaoqi_detected or sleep_detected or audio_file_stack_length > 1:
                print("***************************")
                print("***************************")
                print("no_match_but_valid:",no_match_but_valid)
                print("nihao_detected:", nihao_detected)
            ...
            if response_data['method']=='voice-chat':
                if response_data.get("code") == 0 :
                    ...
                    print("?????_time_get_resp_message:",time.time())
                    print("!!!!resp_message_id:", resp_message_id)
                    ...
                    if str(resp_fomat) == 'mp3':
                        ...
                        while player.poll() is None:  # 检查播放器是否仍在运行
                            if nihao_detected or sleep_detected or xiaoqi_detected or audio_file_stack_length > 1: # or hello_detected: #or result_detected:  # 检查是否需要停止
                                print("#################")
                                print("nihao_detected:", nihao_detected)
                                print("xiaoqi_detected:", xiaoqi_detected)
                        ...
                        if sleep_detected or nihao_detected or xiaoqi_detected or audio_file_stack_length > 1:
                            continue
                        output_audio_stack.append(audio_buffer)
                        output_audio_stack_length = len(output_audio_stack)
                        print("播放栈len:", output_audio_stack_length)
...
def callback(indata, frames, time1, status):
    ...
    global t_play_haode,output_file_name, recording, silence_counter, output_pcm_file, pre_buffer, audio_file_stack, \
        ...
        is_silent_flag,recording_event,flag_think
        ...
        if volume >= SILENCE_THRESHOLD:
            ...
            if recording:
                ...
                if current_time - start_record_time > 1.0 and silence_counter >= 2:
                    print("检测到静音，停止录音")
                    print("?????time_stop_recording:",time.time())
                    recording = False
                    output_pcm_file.close()
                    # process_audio(output_file_name, denoise_output_file_name)
                    if pop_swtich and output_file_name is not None:
...
def audio_run():
    global flag_append,audio_file_stack,flag_len,running2,data_id_no_send,generate_photo_base64,shield_after_send,\
        flag_think,flag_think2,noise_break_flag
    print("audio_run")
    ...
    while running2:
        ...
        time.sleep(0.02)  # 确保事件循环可以调度其他任务
        ...
        #     send_audio_request(audio_file_path)
        # if noise_break_flag==True and flag_len >=1:
        #     print("$$$noise_break_flag=True,clear out audio_file_stack")
        #     # audio_file_stack.clear()
        ...
        if flag_len > 0:
            while len(audio_file_stack) > 1:
                audio_file_stack.pop(0)
            audio_file_path = audio_file_stack.pop(-1)  # 弹出最老的文件
            flag_think=False
            flag_think2=True
...
def start_threads():
    ...
    while running2:
        ...
        with open(log_file_path, "w") as log_file:
            log_file.write(str(1000))  # 将最终值写入文件
            print("Final calculated_pulse written:", 1000)
...
def timer_check():
    global silent_start_time, xiaoqi_detected,recording,pop_swtich,draw_switch,\
        face_detection_switch,xiaoqi_detected_count,sleep_detected,current_pulse_width2
    ...
    while running:
        ...
        if elapsed_time > 180 and elapsed_time < 1000000 or sleep_detected==True:  # 超过25秒静音
            ...
            #
            # if start_angel0-300-50 <= pulse_value <= start_angel0-300+50:  # 判断是否在范围内
            ...
            #
            # # 如果在范围内，执行舵机动作
            # if pulse_within_range and xiaoqi_detected==False:
            #     for i in range(150):
            #         final_pulse = int(current_pulse_width2 + current_step * i)
            #         pwm.setServoPulse(0, final_pulse)
            #         print("final_pulse:",final_pulse)
            #         time.sleep(0.007)
            #
            #     ##将最终脉冲值写入文件
            #     with open(log_file_path, "w") as log_file:
...
SPK_MODEL_PATH = "/home/orangepi/vosk-api/python/example/vosk-model-spk-0.4"
model_path = "/home/orangepi/vosk-model-small-cn-0.22"
...
def kaldi_listener():
    global count2, silent_start_time, silent_end_time, xiaoqi_detected, running2, running,paizhao_voice_command,\
        ...
        nihao_event,is_silent_flag,xiaoqi_event,sleep_event
    ...
    with sd.InputStream(samplerate=args.samplerate, blocksize=8000, device=find_device_index(),
        ...
        while True:
            ...
            if rec.AcceptWaveform(data):
                ...
                if target_keywords[5] in str(partial_text)and not sleep_event_triggered:
                    print(f"检测到sleep关键词: {target_keywords[5]}")
                    # print(f"检测到qibao关键词: {phonemes}")
                    sleep_detected = True
                    sleep_event.set()
                    sleep_event_triggered=True
                    continue
                else:
                    sleep_event.clear()
